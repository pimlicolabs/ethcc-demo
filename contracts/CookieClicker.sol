...
contract CookieClicker {
    ...
    event GameSessionRecorded(
        address indexed player, string username, uint256 cookies, uint256 duration, uint256 timestamp
    );

    // Event emitted when a new best score is achieved
    event NewBestScore(address indexed player, uint256 newBestScore, uint256 previousBestScore);
    ...
    function recordGameSession(uint256 cookies, uint256 duration) external {
        ...
        GameSession memory newSession = GameSession({cookies: cookies, timestamp: block.timestamp, duration: duration});
     ...
     * @return sessionUsernames Array of usernames
     */
    function getTop10GameSessions()
        ...
        returns (
            ...
            string[10] memory sessionUsernames
        )
    {
        ...
        for (uint256 i = 0; i < returnCount; i++) {
            ...
            sessionUsernames[i] = CookieClicker.usernames[player];
        }

        return (players, cookies, timestamps, durations, sessionUsernames);
    }
     ...
     */
    function getPlayerSessionHistory(address player) external view returns (GameSession[] memory sessions) {
