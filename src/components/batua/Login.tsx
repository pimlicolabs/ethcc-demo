...
import { Errors, Provider, WebAuthnP256 } from "ox";
...
import { randomAdjective, randomNoun } from "@/lib/batua/helpers/randomWords";
...
export const Login = ({
	...
	const signIn = useCallback(async () => {
		...
		try {
			...
			const verifiedCredential = await passkeyServerClient.verifyAuthentication(
				{
					...signature,
					uuid: challenge.uuid,
				},
			...
			const { code, errorMessage } = (() => {
				...
				if (error instanceof BaseError || error instanceof Errors.BaseError) {
					const notAllowedError = error.walk(
						(e) => e instanceof Error && e.name === "NotAllowedError",
					);
					...
					const e = error.walk(
						(e) => e instanceof BaseError && e.name === "InternalRpcError",
				...
				return {
					errorMessage: "Failed to sign in, please try using correct passkey",
				};
			})();
