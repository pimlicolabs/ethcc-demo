...
export const Provider = {
	from: ({ internal }: { internal: Internal }) => {
			...
			request: async (request_) => {
				...
				switch (request.method) {
					case "eth_accounts": {
						...
						const response = state.accounts.map((account) => account.address);
						return response;
					}
					case "eth_chainId": {
					...
					case "eth_requestAccounts": {
						...
						const response = accounts.map((account) => account.address);
						return response;
					}
					case "eth_sendTransaction": {
						...
						while (!txHash) {
							const receipts = await getImplementation().actions.getCallsStatus(
								{
									client,
									config,
									request,
					...
					case "wallet_sendCalls": {
						...
						const from =
							request._decoded.params[0].from ?? state.accounts[0].address;
						...
						const hash = await getImplementation().actions.sendCalls({
							...
							calls,
							capabilities:
								request._decoded.params[0]?.capabilities ?? undefined,
							client,
							config,
							request,
							store,
		...
		const setup = () => {
			...
			const unAnnounce =
				internal.config.announceProvider && typeof window !== "undefined"
					? announceProvider({
							info: {
								name: "Batua",
